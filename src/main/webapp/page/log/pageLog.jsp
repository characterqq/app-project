				...
				<div class="panel">
				    ...
				    <div class="panel-body" >
				    	<form id="queryForm" action="" method="post" class="">
							...
							<div class="col-md-2">
								<div class="form-group">
									<input type="text" class="form-control" name="loginPlcae" id="loginPlcae"
										placeholder="登录地址">
								</div>
							</div>
						</form>			    
...
function list(start,limit){
	$.ajax({  
		type : "post",
		url : "<%=basePath%>/log/listLog",  
		data : {
			"start":start,
			"limit":limit,
			"equipmentName":$("#equipmentName").val(),
			"userName":$("#userName").val(),
			"loginPlcae":$("#loginPlcae").val()
						...
						/*  if(item.status==0){
 						...
 						$("#user-table").append("<tr  >"
							...
							/* 	+ "<td class='text-main' style='width: 50px'>" + (item.userName ==null?"":item.userName)+ "</td>" 
								...
								+ "<td class=' text-main' style='width:100px'>"+(item.loginDate ==null?"":item.loginDate)+"</td>" 
